...
from ..core.config import database_name, favorites_collection_name, users_collection_name, place_collection_name
...
async def is_place_favorited_by_user(
    ...
    place_doc = await conn[database_name][place_collection_name].find_one({"slug": slug}, projection={"id": True})
    if place_doc and user_doc:
        ...
        raise RuntimeError(f"没有找到对应的user_id或place_id,"
...
async def remove_place_from_favorites(conn: AsyncIOMotorClient, slug: str, username: str):
    user_doc = await conn[database_name][users_collection_name].find_one({"username": username})
    place_doc = await conn[database_name][place_collection_name].find_one({"slug": slug})
    if place_doc and user_doc:
...
async def get_favorites_count_for_place(conn: AsyncIOMotorClient, slug: str) -> int:
    place_doc = await conn[database_name][place_collection_name].find_one({"slug": slug}, projection={"id": True})
    if place_doc:
        return await conn[database_name][favorites_collection_name].count_documents({"place_id": place_doc['_id']})
    else:
        raise RuntimeError(f"没有找到对应的place_id,"
                           f" slug={slug} place_id={place_doc}")
...
async def update_place_by_slug(
    ...
    await conn[database_name][place_collection_name].replace_one({"slug": slug, "author_id": username}, dbplace.dict())
...
async def delete_place_by_slug(conn: AsyncIOMotorClient, slug: str, username: str):
    await conn[database_name][place_collection_name].delete_many({"author_id": username,
                                                                    "slug": slug})
...
async def get_user_places(
    ...
    place_docs = conn[database_name][place_collection_name].find({"author_id": username},
...
async def get_places_with_filters(
    ...
    rows = conn[database_name][place_collection_name].find({"author_id": username},
                                                             limit=filters.limit,
                                                             skip=filters.offset)
